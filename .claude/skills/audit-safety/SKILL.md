...
{2-3 sentence summary of production-safety posture.}

## Ratings

Use the merit sections above (e.g. "Memory Safety Escape Hatches", "Panics and Unwinding") and apply an individual rating to each with any notes.
...
{Specific things the code does well. File references. Not generic — cite the exact invariant, SAFETY comment, bound, or error path.}
...
{Concrete production-safety findings. Compact ICE. Only suggest remediations for clear gaps.}

### {Finding}
`{file or area}` · {type} · ICE {I}/{C}/{E} → {score} · {trivial | moderate | significant}

{What's wrong and what production failure it would cause. 2-4 sentences.}

**Sketch** (only for high-ICE items):
```{language}
// Brief remediation skeleton — 5-10 lines max
```
...
| # | Gap | Type | Effort | ICE |
| - | --- | ---- | ------ | --- |
| 1 | {description} | {unsafe|panic|leak|concurrency|...} | {effort} | {n.n} |

## Quick Wins

{Top 3 fixes that reduce the most production risk with least effort.}
```

## Calibration Rules

- **Stack rank honestly.** The #1 finding should be the most dangerous production bug class.
- **Do not list more than 15 gaps.** Keep the top 15 by ICE score.
- **Every gap must be specific.** Not "reduce unwrap usage" but "`actor.rs:412` unwraps a closed channel send and will panic the MCP server on client disconnect."
- **Sketches only for high-ICE items.** Don't pad the output with boilerplate remediations for every gap.
- **Documented invariants beat silent unsafety.** An `unsafe` block without a `// SAFETY:` comment is always at least medium ICE when the surrounding proof is non-obvious.
